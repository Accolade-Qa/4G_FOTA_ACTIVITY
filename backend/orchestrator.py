...
from typing import Optional, List, Set, Tuple, Any, Callable
...
class FotaAsyncTriggerWorker(threading.Thread):
    ...
    def __init__(
        ...
        device_state: str,
        on_finished: Optional[Callable[[bool, str, str, str, dict], None]] = None
    ) -> None:
        ...
        try:
            ...
            if first_item:
                ...
                if mismatches:
                    ...
                    self.orchestrator.emit_status(mismatch_msg)
                ...
                if self.on_finished:
                    self.on_finished(True, target_ver, "SCANNED_HISTORY", msg, first_item)
            ...
            if not next_ver:
                if not self.orchestrator.resolver.validate_version_exists(self.device_state, self.login_info.version):
                    ...
                    self.orchestrator.emit_status(toast_msg)
                else:
                ...
                if self.on_finished:
                    self.on_finished(False, "", "BLOCKED", msg, {})
                return
            ...
            if self.on_finished:
                self.on_finished(success, next_ver, api_msg, msg, {})

        except Exception as err:
            logger.error("Async FOTA trigger worker exception: %s", err)
            if self.on_finished:
                self.on_finished(False, "", "ERROR", str(err), {})


class FotaApiPollerWorker(threading.Thread):
    """Background polling thread that monitors active FOTA status from API.
    Polling frequency:
    - 60 seconds when progress < 95%
    - 3 seconds when progress >= 95% till 100% or completion/abort
    """
            ...
            try:
                ...
                if history and isinstance(history, list) and len(history) > 0:
                    ...
                    if attempt_count >= 3:
                        ...
                        logger.warning(msg)
                        if self.on_finished:
                            self.on_finished("ATTEMPTS_EXCEEDED", msg)
                        break
                    ...
                    if is_completed or progress >= 100.0:
                        if not completed_emitted:
                            ...
                            logger.info(msg)
                            if self.on_finished:
                                self.on_finished("COMPLETED", msg)
                    ...
                    # Determine adaptive polling delay: 60s if < 95%, 3s if >= 95% till Stage 10 completes!
...
class FotaOrchestrator:
    ...
    def __init__(
        ...
        on_device_info: Optional[Callable[[LoginPacketInfo], None]] = None,
        on_audit_record: Optional[Callable[[FotaAuditRecord], None]] = None,
        on_request_command: Optional[Callable[[str], None]] = None,
        on_stage_change: Optional[Callable[[int, str, str], None]] = None,
    ) -> None:
        ...
        self.on_audit_record = on_audit_record
        self.on_request_command = on_request_command
    ...
    def emit_device_info(self, info: LoginPacketInfo) -> None:
        if self.on_device_info:
            self.on_device_info(info)

    def emit_audit_record(self, record: FotaAuditRecord) -> None:
        if self.on_audit_record:
            self.on_audit_record(record)

    def emit_request_command(self, cmd: str) -> None:
        if self.on_request_command:
            self.on_request_command(cmd)

    def emit_stage(self, stage: int, status: str, msg: str) -> None:
        if self.on_stage_change:
            self.on_stage_change(stage, status, msg)
    ...
    def _evaluate_api_skipped_stages(self) -> None:
        ...
        if self.download_100_reached or self.reboot_detected or is_completed_api:
            ...
            if not self.reboot_detected or self.stage_states.get(6) != "PASSED":
                ...
                self.emit_stage(6, "PASSED", "Post-installation device reboot confirmed via API")
            ...
            if not self.state_ota_verified:
                if swemp_ok or is_completed_api:
                    ...
                    self.emit_stage(7, "PASSED", msg)
                else:
                    self.stage_states[7] = "RUNNING"
                    self.emit_stage(7, "RUNNING", f"Waiting for State Enable OTA Status to be Set/Skipped in API (Current: {state_status or 'Pending'})...")
                    return
            ...
            if self.state_ota_verified and not self.ip1_verified:
                if p_ok or is_completed_api:
                    ...
                    self.emit_stage(8, "PASSED", msg)
                else:
                    self.stage_states[8] = "RUNNING"
                    self.emit_stage(8, "RUNNING", f"Waiting for Primary IP Status to be Set/Skipped in API (Current: {p_status or 'Pending'})...")
                    return
            ...
            if self.ip1_verified and not self.ip2_verified:
                if s_ok or is_completed_api:
                    ...
                    self.emit_stage(9, "PASSED", msg)
                    self.stage_states[10] = "RUNNING"
                    self.emit_stage(10, "RUNNING", "Validating 55AA Login Packet post-upgrade firmware version...")
                else:
                    self.stage_states[9] = "RUNNING"
                    self.emit_stage(9, "RUNNING", f"Waiting for Secondary IP Status to be Set/Skipped in API (Current: {s_status or 'Pending'})...")
                    return
    ...
    def prepare_next_fota_cycle(self) -> None:
        ...
        logger.info("Preparing next sequential FOTA cycle.")
        ...
        self.stage_states = {s: "WAITING" for s in range(1, 11)}
        for s in range(1, 11):
            self.emit_stage(s, "WAITING", "Waiting for next cycle...")
    ...
    def reset_orchestrator(self) -> None:
        """Reset orchestrator state upon telemetry clearing."""
        if self.poller_worker:
            self.poller_worker.stop()
            self.poller_worker = None
        ...
        self.stage_states = {s: "WAITING" for s in range(1, 11)}
        for s in range(1, 11):
            self.emit_stage(s, "WAITING", "Reset")
        self.clr_fota_ok_received = False
    ...
    def initialize_system(self) -> bool:
        ...
        if ok:
            self.emit_status("API Matrix Sync complete. System Ready.")
        else:
            self.emit_status("System Ready (using local cached state matrix).")
        return ok
    ...
    def process_log_line(self, line: str) -> None:
        ...
        if cip2_val and not self.cip2_validated:
            if self.target_qa_domain in cip2_val.lower():
                ...
                self.emit_status(f"CIP2 Server Verified: {cip2_val}")
            else:
                ...
                self.emit_status(f"CIP2 set to {cip2_val}. Auto-firing QA setup: {cmd}")
                self.emit_request_command(cmd)
                self.cip2_validated = True
        ...
        if self.download_100_reached and not self.reboot_detected:
            if MessageParser.is_device_reboot(line):
                self.reboot_detected = True
                self.stage_states[6] = "PASSED"
                self.emit_stage(6, "PASSED", "Post-installation device reboot/reset detected (120s window active)")
                self.stage_states[7] = "RUNNING"
                self.emit_stage(7, "RUNNING", "Validating SWEMP State Enabled OTA...")
        ...
        if self.reboot_detected and not self.state_ota_verified:
            if not has_state_enable:
                ...
                self.emit_stage(7, "PASSED", msg)
                self.stage_states[8] = "RUNNING"
                self.emit_stage(8, "RUNNING", "Validating Primary Server CHTP IP1 & Port1...")
            else:
        ...
        if self.state_ota_verified and not self.ip1_verified:
            if not has_ip1:
                ...
                self.emit_stage(8, "PASSED", msg)
                self.stage_states[9] = "RUNNING"
                self.emit_stage(9, "RUNNING", "Validating Secondary Server CIP1 IP2 & Port2...")
            else:
        ...
        if self.ip2_verified and not self.config_verified:
            if not self.prncfg_command_fired:
                ...
                self.emit_stage(10, "RUNNING", "Stage 10 active. Waiting 60s post-upgrade delay before firing *GET#PRNCFG#...")
                threading.Timer(60.0, self._fire_stage10_prncfg_command).start()
    ...
    def _fire_stage10_prncfg_command(self) -> None:
        ...
        if self.is_upgrading and not self.config_verified:
            ...
            self.prncfg_response_received = False
            self.emit_request_command("*GET#PRNCFG#")
            self.emit_stage(10, "RUNNING", "Fired *GET#PRNCFG#. Validating telemetry & post-upgrade firmware version...")
    ...
    def _evaluate_stage10_completion(self, current_log_line: str = "") -> None:
        ...
        if self.ip2_verified and uin_ok and imei_ok and vin_ok and login_telemetry_ok and prncfg_ok and version_matches:
            ...
            self.emit_stage(10, "PASSED", f"Post-upgrade 55AA Login Packet & PRNCFG log verified target '{ver_label}'")
            ...
            threading.Timer(4.0, self.prepare_next_fota_cycle).start()
    ...
    def process_login_packet(self, login_info: LoginPacketInfo, selected_ui_state: str = "") -> bool:
        ...
        self.emit_device_info(login_info)
            ...
            if not self.is_upgrading and not self.config_verified:
                for s in range(1, 11):
                    if self.stage_states.get(s) != "WAITING":
                        ...
                        self.emit_stage(s, "WAITING", "Waiting for complete telemetry...")
            ...
            if saved_prog >= 100.0:
                ...
                for s_num, s_state in saved_stages.items():
                    ...
                    if s_state in ("PASSED", "RUNNING"):
                        self.emit_stage(s_int, s_state, f"Restored Stage {s_int} ({s_state}) from active session")

                self.emit_progress(saved_prog)
                self.download_100_reached = True

        # Stage 1: Telemetry Params Passed
        self.stage_states[1] = "PASSED"
        self.emit_stage(1, "PASSED", f"Telemetry abstracted (UIN: {login_info.uin})")
        ...
        if is_valid_ver:
            ...
            self.emit_stage(2, "PASSED", f"State version '{login_info.version}' validated in '{device_state}'")
        else:
            self.stage_states[2] = "FAILED"
            self.emit_stage(2, "FAILED", f"Version '{login_info.version}' not listed in '{device_state}' matrix")
            warning_msg = f"⚠️ Version Warning: Device version '{login_info.version}' not found in '{device_state}' server matrix!"
            logger.warning(warning_msg)
            self.emit_status(warning_msg)
        ...
        self.emit_status("Abstracted all 5 telemetry fields! Checking active FOTA history in background...")

        self._trigger_worker = FotaAsyncTriggerWorker(self, login_info, device_state, on_finished=self._on_trigger_finished)
    ...
    def _on_trigger_finished(self, success: bool, target_ver: str, api_msg: str, status_msg: str, raw_item: dict) -> None:
        """Callback executed when async trigger worker completes."""
        ...
        if success and raw_item and api_msg == "SCANNED_HISTORY":
            if self.current_device:
                ...
                if is_completed:
                    ...
                    for s in range(1, 11):
                        ...
                        self.emit_stage(s, "PASSED", f"Stage {s} verified completed from server history")
                    ...
                    if scanned_progress < 100.0:
                        ...
                        for s in range(5, 11):
                            self.stage_states[s] = "WAITING"
                            self.emit_stage(s, "WAITING", "Waiting for FOTA download completion...")

                    if not self.poller_worker or not self.poller_worker.is_alive():
                        self.poller_worker = FotaApiPollerWorker(
                            self,
                            self.current_device.imei,
                            on_update=self._on_poller_update,
                            on_finished=self._on_poller_finished
                        )
            ...
            for s in range(3, 11):
                self.stage_states[s] = "WAITING"
                self.emit_stage(s, "WAITING", f"Waiting for FOTA download completion (Target: {target_ver})...")
            ...
            if self.current_device:
                ...
                self.emit_status(in_progress_msg)
            ...
            if self.current_device and api_msg != "BLOCKED":
                ...
                if "already at the latest firmware level" in status_msg.lower():
                    ...
                    for s in range(1, 11):
                        self.stage_states[s] = "PASSED"
                        self.emit_stage(s, "PASSED", f"Already at latest firmware level ({self.current_device.version})")
                else:
    ...
    def _on_poller_finished(self, status_type: str, msg: str) -> None:
        ...
        if self.current_device and self.target_version:
            if status_type == "COMPLETED":
                ...
                self.emit_status(f"⛔ FOTA Aborted. Audit report saved. Re-initiating FOTA API request for {self.current_device.uin}...")

                dev = self.current_device
                state = dev.state
                self.attempted_uins.discard(dev.uin)

                self._trigger_worker = FotaAsyncTriggerWorker(self, dev, state, on_finished=self._on_trigger_finished)
                self._trigger_worker.start()
    ...
    def update_progress(self, progress: float) -> None:
        ...
        if val_clean >= 100.0:
            ...
            if self.stage_states.get(5) != "PASSED":
                ...
                self.emit_stage(5, "PASSED", "100.00% Downloaded")
                self.stage_states[6] = "RUNNING"
                self.emit_stage(6, "RUNNING", "Monitoring for device reboot...")
            ...
            self.emit_status(f"FOTA Downloading: {val_clean:.2f}%")
